...
const EliminarProveedor = ({ proveedorActual, onGuardar, onCancelar }) => {
  const [show, setShow] = useState(true);

  const handleGuardar = () => {
   
    onGuardar(proveedorActual); // Ejecuta onGuardar y cierra el modal
    setShow(false);
  };
    ...
    style={{ display: 'block', position: 'initial' }}>
      ...
      <Modal.Header closeButton style={{ backgroundColor: '#007bff', color: 'white' }}>
        ...
        <Modal.Body style={{ borderRadius: '20px', width: '100%', alignContent:'center' }}>
          <p>¿Está seguro de eliminar al proveedor?</p>
       
          <Row style={{ borderRadius: '20px', width: '100%' , justifyContent: 'center'}}>
             {proveedorActual[1]}</Row>
